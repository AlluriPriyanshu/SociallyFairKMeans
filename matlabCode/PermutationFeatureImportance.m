...
for svar1 = 1:4
    for svar2 = svar1+1:5
        for PAEcol = 1:4
            ...
            if PAEcol ~= 4
                PAEdata = dataAll(:,PAEcol);
                PAEdata = PAEdata(randperm(length(PAEdata)));
                dataAll(:,PAEcol) = PAEdata;
            end
            bestOutOf = 10;
            numIters = 10;
            numCenters = 3;
            ...
            PAEcostNF{svar1,svar2}(PAEcol) = CostNF{3,1}(1);
        end

        svar1_name = ethnicity_dict(svar1);
        svar2_name = ethnicity_dict(svar2);
        output_title = [datasetName, '_', svar1_name, 'vs', svar2_name];
        save([output_title, 'PAE']);
    end
end



x = categorical({'Diener', 'PHQ-9', 'GAD-7','None'});
x = reordercats(x,{'Diener', 'PHQ-9', 'GAD-7','None'}); 
x = x(:);
...
for svar1 = 1:4
    for svar2 = svar1+1:5
        ...
        bar(x,y)% 'o-' means circles connected by lines
        xlabel('Feature');
        ylabel('Costs');
        ylim([1 4])
        hold on;
        ...
        title(['PFI feature cost ', ethnicity_dict(svar1),' and ', ethnicity_dict(svar2)]);
        xticklabels(x);
        exportgraphics(gcf,['PFI', ethnicity_dict(svar1),'and', ethnicity_dict(svar2),'.png',],'Resolution',300)
        %grid on;

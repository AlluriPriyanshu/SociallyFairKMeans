...
for svar1 = 1:4
    for svar2 = svar1+1:5
        ...
        load(filename);
        [costCheck,nonfairClusterings] = bootstrappedCostCalculator(datasep,centersN);
        numCenters = 3;
        ...
        for i = 1:k
            for j = 1:5
                raceCounts(i, j) = sum((nonfairClusterings{j} == i));
            end
        end
        ...
        for j = 1:5
            totalRaceCounts(j) = length(nonfairClusterings{j});
        end

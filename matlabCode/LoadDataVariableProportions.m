function [dataAll, svarAll, groupNames, varsOfInt] = loadData( datasetName, svar1, svar2 )
    ...
    elseif strcmp(datasetName, 'HMS')
        ...
        varsUnordered = csvread("C:/Users/allur/OneDrive - Dartmouth College/Desktop/Marreo Lab Work/Socially Fair K means/HMS/varProp.csv");

        logical_index = (dataAll(:, 4) == svar1) | (dataAll(:, 4) == svar2);

        dataAll = dataAll(logical_index, :);
        varsOfInt = varsUnordered(logical_index, :);
        ...
        logical_index = (svarAll(:,1) == svar1) | (svarAll(:, 1) == svar2);
        % Use the logical index to select the subset of rows from dataAll
        ...
        changeToTwo = (svarAll(:,1) == svar2);
        svarAll(changeToTwo,1) = 2;
        ...
        groupNames = {ethnicity_dict(svar1), ethnicity_dict(svar2)}; %Make dictionary in the future
    end
end

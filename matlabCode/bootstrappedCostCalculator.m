%IF THERE IS AN ERROR; STOP RETURNING CLUSTERING -- THAT SHOULD FIX IT!!
function [costs, clustering] = bootstrappedCostCalculator(sepData, centers)
    
    disp(centers)
    %calcualtes the costs for each different race from the fair centers
    %generated for two groups -- Created by Priyanshu Alluri
    %Takes centers for each variable
    temp = centers{3,1};
    centers{3} = temp;
